  ...
  IERC165
{
  ...
  /// @notice Value used as divisor with the ALTS, commonly used for many financial indices
  ...
  /// @notice An event emitted when the feeAddress is updated
  ...
  /**
   * @notice Sets the feeAddress where fees charged by the protocol are transfered to
   * @param _feeAddress address
   * @dev Only the current feeAddress can call it
   */
  function setFeeAddress(address _feeAddress) public virtual {
    require(msg.sender == feeAddress, "VaultHandler::setFeeAddress: this address is not allowed to set the new dev address");
   ...
   * @dev oracle altsPrice must be in wei format
   * @dev P = V / d
   * P = ALTS Token Price
   * V = ALTS price
   * d = Divisor
