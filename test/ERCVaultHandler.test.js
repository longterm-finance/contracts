var { expect, before } = require('chai')
...
describe('ERC20 Vault', async function () {
  ...
  it('...should allow users to create a vault', async () => {
    ...
    expect(vaultId).eq(0)
    await expect(
      ercTokenHandler.connect(addr1).createVault(),
  ...
  it('...should allow user to stake collateral', async () => {
    ...
    await expect(
      ercTokenHandler.connect(addr1).addCollateral(0),
  ...
  it('...should allow user to retrieve unused collateral', async () => {
    ...
    await expect(
      ercTokenHandler.connect(addr3).removeCollateral(amount),
    ).to().be.revertedWith('VaultHandler::vaultExists: no vault created')
    await expect(
      ercTokenHandler.connect(addr1).removeCollateral(bigAmount),
    ).to().be.revertedWith(
      'VaultHandler::removeCollateral: retrieve amount higher than collateral',
    )
    await expect(
      ercTokenHandler.connect(addr1).removeCollateral(0),
    ).to().be.revertedWith("VaultHandler::notZero: value can't be 0")
  ...
  it("...shouldn't allow minting above cap", async () => {
    ...
    await expect(
      ercTokenHandler.connect(addr1).mint(reqAmount),
    ).to().be.revertedWith('TCAP::Transfer: TCAP cap exceeded')
    // Remove Cap
    enableCap = false
    await orchestratorInstance.enableTCAPCap(tcapInstance.address, enableCap)
  })
  ...
  it('...should allow user to mint tokens', async () => {
    ...
    await expect(
      ercTokenHandler.connect(addr3).mint(amount),
    ).to().be.revertedWith('VaultHandler::vaultExists: no vault created')
    await expect(
      ercTokenHandler.connect(addr1).mint(bigAmount),
    ...
    await expect(
      ercTokenHandler.connect(addr1).mint(lowAmount),
    ).to().be.revertedWith(
  ...
  it("...shouln't allow users to retrieve stake unless debt is paid", async () => {
    let vault = await ercTokenHandler.getVault(1)
    await expect(
      ercTokenHandler.connect(addr1).removeCollateral(vault[1]),
    ).to().be.revertedWith(
  ...
  it('...should allow users to burn tokens', async () => {
    ...
    await expect(
      ercTokenHandler.connect(addr3).burn(amount),
    ).to().be.revertedWith('VaultHandler::vaultExists: no vault created')
    await expect(
      ercTokenHandler.connect(addr1).burn(amount),
  ...
  it('...should test liquidation requirements', async () => {
    ...
    await expect(
      ercTokenHandler.connect(addr3).liquidateVault(99, 0),
    ).to().be.revertedWith('VaultHandler::liquidateVault: no vault created')
    await expect(
      ercTokenHandler.connect(addr3).liquidateVault(2, 0),
    ).to().be.revertedWith(
