...
describe('ERC20 Vault', async function () {
  ...
  it('...should allow the owner to set the treasury address', async () => {
    ...
    await expect(
      ...
      .to()
      .emit(ercTokenHandler, 'NewTreasury')
      .withArgs(orchestratorInstance.address, treasury)

    expect(await ercTokenHandler.treasury())
      .to()
      .eq(treasury)
  })

  it('...should return the token price', async () => {
    let vixPrice = await dvixOracleInstance.getLatestAnswer()
    let result = vixPrice.mul(10000000000).div(divisor)
    expect(vixPrice).to().eq(result)
  })
  ...
  it('...should allow users to create a vault', async () => {
    ...
    await expect(ercTokenHandler.connect(addr1).createVault())
      .to()
      .emit(ercTokenHandler, 'VaultCreated')
  ...
  it('...should allow user to stake collateral', async () => {
    ...
    await expect(ercTokenHandler.connect(addr3).addCollateral(amount))
      .to()
    ...
    await ercTokenInstance
      .connect(addr1)
      .approve(ercTokenHandler.address, amount)
    balance = await ercTokenInstance.balanceOf(accounts[1])
    expect(balance).to().eq(amount)

    await expect(ercTokenHandler.connect(addr1).addCollateral(0))
      .to()
      .be.revertedWith("VaultHandler::notZero: value can't be 0")

    await expect(ercTokenHandler.connect(addr1).addCollateral(amount))
      .to()
      .emit(ercTokenHandler, 'CollateralAdded')
  ...
  it('...should allow user to retrieve unused collateral', async () => {
    ...
    await expect(ercTokenHandler.connect(addr3).removeCollateral(amount))
      .to()
      .be.revertedWith('VaultHandler::vaultExists: no vault created')
    await expect(ercTokenHandler.connect(addr1).removeCollateral(bigAmount))
      .to()
      .be.revertedWith(
    ...
    await expect(ercTokenHandler.connect(addr1).removeCollateral(amount))
      .to()
      .emit(ercTokenHandler, 'CollateralRemoved')
      .withArgs(accounts[1], 1, amount)
  ...
  it('...should return the correct minimal collateral required', async () => {
    ...
    const dvixPrice = await ercTokenHandler.DVIXPrice()
    const ratio = await ercTokenHandler.ratio()
    let result = dvixPrice
      .mul(amount)
      .mul(ratio)
      .div(100)
      .div(ethPrice.mul(10000000000))
  ...
  it("...shouldn't allow minting above cap", async () => {
    ...
    await ercTokenInstance
      .connect(addr1)
      .approve(ercTokenHandler.address, reqAmount)
    ...
    let dvixCap = 1
    await orchestratorInstance.enableDVIXCap(dvixInstance.address, enableCap)
    await orchestratorInstance.setDVIXCap(dvixInstance.address, dvixCap)
    await expect(ercTokenHandler.connect(addr1).mint(reqAmount))
      .to()
      .be.revertedWith('DVIX::Transfer: DVIX cap exceeded')
    // Remove Cap
    enableCap = false
    await orchestratorInstance.enableDVIXCap(dvixInstance.address, enableCap)
  })
  ...
  it('...should allow user to mint tokens', async () => {
    ...
    await expect(ercTokenHandler.connect(addr3).mint(amount))
      .to()
      .be.revertedWith('VaultHandler::vaultExists: no vault created')
    await expect(ercTokenHandler.connect(addr1).mint(bigAmount))
      .to()
      .be.revertedWith('VaultHandler::mint: not enough collateral')
    await expect(ercTokenHandler.connect(addr1).mint(0))
      .to()
      .be.revertedWith("VaultHandler::notZero: value can't be 0")
    await expect(ercTokenHandler.connect(addr1).mint(amount))
      .to()
      .emit(ercTokenHandler, 'TokensMinted')
      .withArgs(accounts[1], 1, amount)
    ...
    await expect(ercTokenHandler.connect(addr1).mint(lowAmount))
      .to()
      .be.revertedWith(
  ...
  it('...should allow token transfers', async () => {
    let dvixBalance = await dvixInstance.balanceOf(accounts[1])
    await dvixInstance.connect(addr1).transfer(accounts[2], dvixBalance)
    expect(await dvixInstance.balanceOf(accounts[1]))
      .to()
      .eq(0)
    expect(await dvixInstance.balanceOf(accounts[2]))
      .to()
      .eq(dvixBalance)
    await dvixInstance.connect(addr2).transfer(accounts[1], dvixBalance)
    expect(await dvixInstance.balanceOf(accounts[2]))
      .to()
      .eq(0)
    expect(await dvixInstance.balanceOf(accounts[1]))
  ...
  it("...shouldn't allow user to send tokens to dvix contract", async () => {
    let dvixBalance = await dvixInstance.balanceOf(accounts[1])
    await expect(
      dvixInstance.connect(addr1).transfer(dvixInstance.address, dvixBalance),
    )
      .to()
      .be.revertedWith("DVIX::transfer: can't transfer to DVIX contract")
  })
  ...
  it('...should calculate the burn fee', async () => {
    ...
    let divisor = 100
    let dvixPrice = await ercTokenHandler.DVIXPrice()
  ...
  it('...should allow users to burn tokens', async () => {
    ...
    const reqAmount2 = await ercTokenHandler.requiredCollateral(amount2)
    // eslint-disable-next-line
    const avaxAmount = await ercTokenHandler.getFee(amount)
    const avaxAmount2 = await ercTokenHandler.getFee(bigAmount)

    await expect(ercTokenHandler.connect(addr3).burn(amount))
      .to()
      .be.revertedWith('VaultHandler::vaultExists: no vault created')
    await expect(ercTokenHandler.connect(addr1).burn(amount))
      .to()
      .be.revertedWith('VaultHandler::burn: burn fee less than required')
    await expect(
      ercTokenHandler.connect(addr1).burn(bigAmount, { value: avaxAmount2 }),
    )
      .to()
      .be.revertedWith('VaultHandler::burn: amount greater than debt')

    await expect(ercTokenHandler.connect(addr1).burn(0))
      .to()
      .be.revertedWith("VaultHandler::notZero: value can't be 0")
    await expect(
      ...
      .to()
      .emit(ercTokenHandler, 'TokensBurned')
      .withArgs(accounts[1], 1, amount)
    let dvixBalance = await dvixInstance.balanceOf(accounts[1])
    expect(dvixBalance).to().eq(0)
    let vault = await ercTokenHandler.getVault(1)
  ...
  it('...should test liquidation requirements', async () => {
    ...
    await expect(ercTokenHandler.connect(addr3).liquidateVault(99, 0))
      .to()
      .be.revertedWith('VaultHandler::liquidateVault: no vault created')
    await expect(ercTokenHandler.connect(addr3).liquidateVault(2, 0))
      .to()
  ...
  it('...should get the required collateral for liquidation', async () => {
    let reqLiquidation = await ercTokenHandler.requiredLiquidationDVIX(2)
  ...
  it('...should allow liquidators to return profits', async () => {
    ...
    const reqLiquidation = await ercTokenHandler.requiredLiquidationDVIX(2)
    const dvixPrice = await ercTokenHandler.DVIXPrice()
  ...
  it('...should allow users to liquidate users on vault ratio less than ratio', async () => {
    ...
    await ercTokenInstance
      .connect(addr3)
      .approve(ercTokenHandler.address, reqLiquidatorAmount)
    ...
    await expect(
      ...
      .to()
      .emit(ercTokenHandler, 'VaultLiquidated')
    ...
    let currentEthBalance = await ethers.provider.getBalance(
      ercTokenHandler.address,
    )
  ...
  it('...should allow owner to pause contract', async () => {
    await expect(ercTokenHandler.connect(addr1).pause())
      .to()
      .be.revertedWith('Ownable: caller is not the owner')
    await expect(
      orchestratorInstance
        .connect(guardian)
        .pauseVault(ercTokenHandler.address),
    )
      .to()
      .emit(ercTokenHandler, 'Paused')
  ...
  it('...should allow owner to unpause contract', async () => {
    await expect(ercTokenHandler.connect(addr1).unpause())
      .to()
      .be.revertedWith('Ownable: caller is not the owner')
    await expect(
      orchestratorInstance
        .connect(guardian)
        .unpauseVault(ercTokenHandler.address),
    )
      .to()
      .emit(ercTokenHandler, 'Unpaused')
      .withArgs(orchestratorInstance.address)
    let paused = await ercTokenHandler.paused()
