...
describe('ETH Vault', async function () {
  ...
  it('...should deploy the contract', async () => {
    ...
    const aggregatorTcap = await ethers.getContractFactory(
  ...
  it('...should return the token price', async () => {
    ...
    let totalMarketCap = (await tcapOracleInstance.getLatestAnswer()).mul(
      10000000000,
    )
  ...
  it('...should allow users to create a vault', async () => {
    ...
    expect(vaultId).eq(0)
    await expect(
      ethTokenHandler.connect(addr1).createVault(),
    ).to.be.revertedWith('VaultHandler::createVault: vault already created')
  })
  ...
  it('...should allow user to stake weth collateral', async () => {
    ...
    await expect(
      ethTokenHandler.connect(addr3).addCollateral(amount),
    ).to.be.revertedWith('VaultHandler::vaultExists: no vault created')
    let balance = await wethTokenInstance.balanceOf(accounts[1])
    expect(balance).to.eq(0)

    await expect(
      ethTokenHandler.connect(addr1).addCollateral(amount),
    ).to.be.revertedWith('')
    ...
    await wethTokenInstance
      .connect(addr1)
      .approve(ethTokenHandler.address, amount)
    balance = await wethTokenInstance.balanceOf(accounts[1])
    expect(balance).to.eq(amount)

    await expect(
      ethTokenHandler.connect(addr1).addCollateral(0),
    ).to.be.revertedWith("VaultHandler::notZero: value can't be 0")
  ...
  it('...should allow user to stake eth collateral', async () => {
    ...
    await expect(
      ethTokenHandler.connect(addr1).addCollateralETH(),
    ).to.be.revertedWith("ETHVaultHandler::addCollateralETH: value can't be 0")

    await expect(
      ethTokenHandler.connect(addr1).addCollateralETH({ value: amount }),
    )
      .to.emit(ethTokenHandler, 'CollateralAdded')
      .withArgs(accounts[1], 1, amount)
    vault = await ethTokenHandler.getVault(1)
  ...
  it('...should allow user to retrieve unused collateral on eth', async () => {
    ...
    let vault = await ethTokenHandler.getVault(1)
    let vaultBalance = vault[1]
    let contractBalance = await wethTokenInstance.balanceOf(
      ethTokenHandler.address,
    )
    await expect(
      ethTokenHandler.connect(addr3).removeCollateralETH(amount),
    ).to.be.revertedWith('VaultHandler::vaultExists: no vault created')
    await expect(
      ethTokenHandler.connect(addr1).removeCollateralETH(bigAmount),
    ).to.be.revertedWith(
      'ETHVaultHandler::removeCollateralETH: retrieve amount higher than collateral',
    )
    await expect(
      ethTokenHandler.connect(addr1).removeCollateralETH(0),
    ).to.be.revertedWith(
  ...
  it('...should allow user to retrieve unused collateral on weth', async () => {
    ...
    await expect(
      ethTokenHandler.connect(addr3).removeCollateral(amount),
    ).to.be.revertedWith('VaultHandler::vaultExists: no vault created')
    await expect(
      ethTokenHandler.connect(addr1).removeCollateral(bigAmount),
    ).to.be.revertedWith(
      'VaultHandler::removeCollateral: retrieve amount higher than collateral',
    )
    await expect(
      ethTokenHandler.connect(addr1).removeCollateral(0),
    ).to.be.revertedWith("VaultHandler::notZero: value can't be 0")
  ...
  it('...should return the correct minimal collateral required', async () => {
    ...
    const ethPrice = (await priceOracleInstance.getLatestAnswer()).mul(
      10000000000,
    )
  ...
  it('...should allow to earn fees if reward address is set', async () => {
    ...
    expect(
      await rewardTokenInstance.balanceOf(rewardHandlerInstance.address),
    ).to.eq(rewardAmount)
  })
  ...
  it('...should allow user to mint tokens', async () => {
    ...
    await wethTokenInstance
      .connect(addr1)
      .approve(ethTokenHandler.address, reqAmount2)
    await ethTokenHandler.connect(addr1).addCollateral(reqAmount2)
    await expect(
      ethTokenHandler.connect(addr3).mint(amount),
    ).to.be.revertedWith('VaultHandler::vaultExists: no vault created')
    await expect(
      ethTokenHandler.connect(addr1).mint(bigAmount),
    ).to.be.revertedWith('VaultHandler::mint: not enough collateral')
    ...
    await expect(
      ethTokenHandler.connect(addr1).mint(lowAmount),
    ).to.be.revertedWith(
  ...
  it("...shouln't allow users to retrieve stake unless debt is paid", async () => {
    let vault = await ethTokenHandler.getVault(1)
    await expect(
      ethTokenHandler.connect(addr1).removeCollateral(vault[1]),
    ).to.be.revertedWith(
  ...
  it('...should calculate the burn fee', async () => {
    ...
    let divisor = 100
    let tcapPrice = await ethTokenHandler.TCAPPrice()
    let ethPrice = (await priceOracleInstance.getLatestAnswer()).mul(
      10000000000,
    )
  ...
  it('...should allow users to burn tokens', async () => {
    ...
    await expect(
      ethTokenHandler.connect(addr3).burn(amount),
    ).to.be.revertedWith('VaultHandler::vaultExists: no vault created')
    await expect(
      ethTokenHandler.connect(addr1).burn(amount),
    ).to.be.revertedWith('VaultHandler::burn: burn fee less than required')
    await expect(
    ...
    await expect(
      ethTokenHandler.connect(addr1).burn(amount, { value: ethAmount }),
    )
  ...
  it('...should test liquidation requirements', async () => {
    ...
    await wethTokenInstance
      .connect(lq)
      .approve(ethTokenHandler.address, reqAmount)
    await ethTokenHandler.connect(lq).addCollateral(reqAmount)
    await ethTokenHandler.connect(lq).mint(amount)
    await expect(
      ethTokenHandler.connect(addr3).liquidateVault(99, 0),
    ).to.be.revertedWith('VaultHandler::liquidateVault: no vault created')
    await expect(
      ethTokenHandler.connect(addr3).liquidateVault(2, 0),
    ).to.be.revertedWith(
  ...
  it('...should allow users to liquidate users on vault ratio less than ratio', async () => {
    ...
    await wethTokenInstance
      .connect(addr3)
      .approve(ethTokenHandler.address, reqLiquidatorAmount)
  ...
  it('...should allow owner to pause contract', async () => {
    ...
    await expect(
      orchestratorInstance
        .connect(guardian)
        .pauseVault(ethTokenHandler.address),
    )
  ...
  it("...shouldn't allow contract calls if contract is paused", async () => {
    ...
    await expect(ethTokenHandler.connect(addr1).mint(0)).to.be.revertedWith(
      'Pausable: paused',
    )
    await expect(
      ethTokenHandler.connect(addr1).removeCollateral(0),
  ...
  it('...should allow owner to unpause contract', async () => {
    ...
    await expect(
      orchestratorInstance
        .connect(guardian)
        .unpauseVault(ethTokenHandler.address),
    )
